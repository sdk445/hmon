...
HMON_DECLARE_PLUGIN("docker")

static hmon::plugins::docker::DockerPluginCtx* g_ctx = nullptr;

extern "C" {

HMON_PLUGIN_EXPORT void hmon_plugin_control(const char* key, int value) {
    if (!g_ctx) return;
    if (std::string(key) == "docker.enable") {
        if (value) hmon::plugins::docker::startBackgroundCollector(g_ctx);
        else hmon::plugins::docker::stopBackgroundCollector(g_ctx);
    }
}
